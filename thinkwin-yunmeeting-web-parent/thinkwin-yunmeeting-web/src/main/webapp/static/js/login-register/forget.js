...
$(function(){
    ...
    $("#forgetSend").click(function(){
        ...
        if(phoneNumber==""){
            hideWarning("");
            showWarning("#forgetTel","手机号不能为空");
        }else{
            if(!isPhone(phoneNumber)){
                hideWarning("");
                showWarning("#forgetTel","手机号码格式不正确");
            }else {
...
function fromSubmit() {
    ...
    //2.验证码
    if(code==""){
        showWarning("#forgetCode","验证码不能为空");
    }else{
        if(!checkCode(code)){
    ...
    //4.确认密码
    if(pwdAgain==""){
        showWarning("#forgetPwdAgain","确认密码不能为空");
    }else {
        if(pwd!=pwdAgain){
                ...
                if(data.ifSuc==1){//请求成功 执行跳转成功页面
                    ...
                    if(data.data.param=="phoneNumber"){
                        ...
                        if(data.data.paramState=="isBlank"){ //密码为空
                            showWarning("#inputCompany","密码不能为空");
                        ...
                        if(data.data.paramState=="isBlank"){ //密码为空
                            showWarning("#inputCompany","确认密码不能为空");
                        }else if(data.data.paramState=="isNotFormat"){//密码格式不正确
