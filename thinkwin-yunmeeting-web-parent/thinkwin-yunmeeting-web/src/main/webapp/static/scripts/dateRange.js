...
function pickerDateRange(inputId, options) {
    ...
    $('#' + this.calendarId).css('z-index', 101);
    // 初始化目标地址的元素
    ...
    $('#' + this.submitBtn).bind('click', function () {
        //判断开始结束日期
        //非法空判断
        if($( '#' + __method.calendarId + ' .input_end').val()==''||
            $( '#' + __method.calendarId + ' .input_start').val()==''||
            $( '#' + __method.calendarId + ' .input_start').val().indexOf('-')<=0||
            $( '#' + __method.calendarId + ' .input_end').val().indexOf('-')<=0){
            notify('danger','请输入有效开始或结束时间');
            $('#' + __method.calendarId +' .input_start').val(__method.input_start);
            $( '#' + __method.calendarId + ' .input_end').val(__method.input_end);
            __method.calendar_endDate.setMonth(new Date($('.input_start').val()).getMonth()+1, 1);
            var current = new Date($( '#' + __method.calendarId + ' .input_end').val());
            __method.mOpts.endDate = __method.date2ymd(current).join('-');
            __method.init(0);
            __method.show(false, __method);
            __method.input_start = $( '#' + __method.calendarId + ' .input_start').val();
            __method.input_end = $( '#' + __method.calendarId + ' .input_end').val();
            return;
        }
        var end = new Date($( '#' + __method.calendarId + ' .input_end').val());
        var start = new Date($( '#' + __method.calendarId + ' .input_start').val());
        var sTime = start.getTime();
        var eTime = end.getTime();
        var nowTime = new Date();
        //判断是否是合法时间
        var checkSstart = checkDate($( '#' + __method.calendarId + ' .input_start').val());
        var validStart = moment($( '#' + __method.calendarId + ' .input_start').val()).isValid();
        var validEnd = moment($( '#' + __method.calendarId + ' .input_end').val()).isValid();
        var checkEnd = checkDate($( '#' + __method.calendarId + ' .input_end').val());
        if((isNaN(sTime)  || validStart == false  || checkSstart == false) || (isNaN(eTime) || validEnd == false || checkEnd == false)){
            notify('danger','请输入有效开始或结束时间');
            $('#' + __method.calendarId +' .input_start').val(__method.input_start);
            $( '#' + __method.calendarId + ' .input_end').val(__method.input_end);
            __method.calendar_endDate.setMonth(new Date($('.input_start').val()).getMonth()+1, 1);
    ...
    $('#' + this.todayBtn).bind('click', function () {
        ...
        $( '#' + __method.calendarId + ' .input_end').val(moment(current).format('YYYY-MM-DD'));
    ...
    //日期开始输入框添加输入监听事件
    $('#' + this.calendarId +' .input_start').bind('keypress',function (event) {
        var date = $(this).val().trim();
        if (event.keyCode == 13) {
          var timestamp2 = Date.parse(date);
          //日期合法校验
          if(date.indexOf('-')<=0||date==''){
              notify('danger','请输入有效日期');
              $('#' + __method.calendarId +' .input_start').val(__method.input_start);
              __method.calendar_endDate.setMonth(new Date($('.input_start').val()).getMonth()+1, 1);
              var current = new Date($( '#' + __method.calendarId + ' .input_end').val());
              __method.mOpts.endDate = __method.date2ymd(current).join('-');
          ...
          if (!isNaN(timestamp2) && moment(date).isValid() && checkDate(date)) {
            var sDate = new Date($( '#' + __method.calendarId + ' .input_start').val());
            var eDate = new Date($( '#' + __method.calendarId + ' .input_end').val());
            ...
            if(sTime>nowTime){
                ...
                //给结束时间输入框赋默认值
                $( '#' + __method.calendarId + ' .input_end').val(__method.input_end);
                __method.calendar_endDate.setMonth(new Date($( '#' + __method.calendarId + ' .input_start').val()).getMonth()+1, 1);
                var current = new Date($( '#' + __method.calendarId + ' .input_end').val());
                __method.mOpts.endDate = __method.date2ymd(current).join('-');
                __method.init(0);
                __method.show(false, __method);
    ...
    //日期结束输入框添加输入监听事件
    $('#' + this.calendarId + ' .input_end').bind('keypress',function (event) {
        if (event.keyCode == '13') {
            var date = $(this).val().trim();
            if(date.indexOf('-')<=0 || date == ''){
                notify('danger','请输入有效日期');
...
pickerDateRange.prototype.selectDate = function (ymd) {
    ...
    if (this.startDateId == this.dateInput) {
        ...
        if (-1 == this.compareStrDate(ymd, $('#' + this.startDateId).val())) {
            ...
            this.input_end = $( '#' + this.calendarId + ' .input_end').val();
            // 更改对应输入框的值(开始时间)
            $('#' + this.startDateId).val(ymdFormat);
            this.input_start = $( '#' + this.calendarId + ' .input_start').val();
...
pickerDateRange.prototype.show = function (isCompare, __method) {
    ...
    $("#" + this.calendarId).css('left', left-120 + 'px');
