(function () {
    ...
    Timerange.prototype = {
        init: function () {
           if(this.element.find('input').eq(0).val()>new Date().getTime()){
                var start = this.element.find('input').eq(0).val();
                var end = this.element.find('input').eq(1).val();
            }else{
               var start = new Date(Math.ceil(new Date().getTime() / (15 * 60 * 1000)) * 15 * 60 * 1000).getTime();
            ...
            console.log(new Date('2017/10/27 8:23').getTime())
        },
        place: function () {
        ...
        renderCalendar: function () {
            ...
            if (this.o.datepicker) {
                ...
                this.calendar.children('.datepicker').datepicker({
                    ...
                    endDate: self.o.cycle == 0 ? 0:moment().add(self.o.cycle-1,'days').format('YYYY-MM-DD'),
                    minViewMode: 0,
                    maxViewMode: 0,
                    templates: {
        ...
        renderTimes: function () {
            ...
            if (!this.o.date || new Date(this.o.date).toDateString() == new Date().toDateString()) {
                ...
                start = new Date(date.replace(/-/g,"/") + ' ' + new Date(Math.ceil(new Date().getTime() / this.o.interval) * this.o.interval).toTimeString().slice(0, 5)).getTime();
            } else {
                date = this.o.date;
                start = new Date(date.replace(/-/g,"/") + ' ' + this.o.start).getTime();
            }
            end = new Date(date.replace(/-/g,"/") + ' ' + this.o.end).getTime();
            var timestamp = start;
        ...
        update: function (date) {
            ...
            if (this.start != '' && this.end != '') {
                this.start = new Date(date.replace(/-/g,"/") + ' ' + moment(this.start).format('H:mm')).getTime();
                this.end = new Date(date.replace(/-/g,"/") + ' ' + moment(this.end).format('H:mm')).getTime();
                this.change();
            }
            this.renderTimes();
        },
    ...
    var timerangePlugin = function (option) {
        ...
        this.each(function () {
            ...
            if (!data) {
                var opts = $.extend({}, $.fn.timerange.defaults, options);
