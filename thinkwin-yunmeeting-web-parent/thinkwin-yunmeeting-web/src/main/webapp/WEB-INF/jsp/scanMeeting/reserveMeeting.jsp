...
<head>
	...
	<meta name="format-delection" content="email=no"/>
	  <link rel="stylesheet" href="/static/fonts/mobile/demo.css">
    <link rel="stylesheet" href="/static/fonts/mobile/iconfont.css">
...
<body>
	<div class="box">
		<div class="meetingname">
		    <input type="text" placeholder="请输入会议名称(必填)" id="meetname" maxlength="20">
		</div>
		<div class="newbox">
	      <div class="site">
      ...
      <input type="hidden" value="${meetingRoomId}" id="meetingRoomId" />
        ...
        <div class="newbox phoneDiv">
                  <div class="phone">
		    <p class="iconfont icon-xq-phone createmeetingroomicon"></p>
		     <input type="text" id="number"  placeholder="请输入手机号(必填)"  maxlength="11"  onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)" onblur="this.v();">
		  </div>
		  <div class="password">
		    <p class="iconfont icon-xq-lock createmeetingroomicon"></p>
		    <input type="password" id="password"  placeholder="请输入密码(必填)">
		  </div>
        </div>
		<div class="createbth">
			<p>确定</p>
...
//确定按钮的点击事件
    $(".createbth").click(function () {
      var reg=/^1[34578]\d{9}$/;
      if($("#meetname").val()==""||$("#number").val()==""||$("#password").val()==""){
       layer.open({
          ...
          if (!reg.test($("#number").val())) {
              layer.open({
                  content: '<div class="alertDiv"><div class="alert-icon iconfont icon-false"></div><div class="hint">手机号格式不正确</div></div>'
                  , skin: 'msg'
                  , shade: 0
                  , time: 2 //2秒后自动关闭
              });
              ...
              $.ajax({
                  url : "/logincheck?j_username=" + $('#number').val()+"&j_password="+$('#password').val(),
                  type: "post",
                  success: function (e) {
                      if (e.code == 1) {
                          $.ajax({
                              url: "/yunMeeting/scanMeeting/reserveMeeting",
                              type: "post",
                              data: {
                                  token: e.token,
                                  userId: e.userId,
                          ...
                          layer.open({
                              content: '<div class="alertDiv1"><div class="alert-icon iconfont icon-false"></div><div class="hint">手机号或密码错误</div></div>'
                              , skin: 'msg'
                              , shade: 0
                              , time: 2 //2秒后自动关闭
                          });
